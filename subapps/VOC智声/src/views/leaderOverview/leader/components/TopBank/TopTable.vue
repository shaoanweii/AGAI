...
import CustomTable from '@/views/leaderOverview/leader/common/CustomTable.vue'
...
watch(
  () => props.data,
  newData => {
    theData.value = newData.slice(0, 5)
  },
  { immediate: true }
)
...
const emit = defineEmits<{
  ...
  'row-click': [opinionName: string, intention: string]
}>()
 ...
 */
const handleRowClick = (opinionName: string) => {
  emit('row-click', opinionName, props.intention)
}
...
<template>
  ...
  <el-table
    ...
    class="TopTable"
  >
    <el-table-column prop="opinionName" label="观点" />
    <el-table-column prop="mentionCount" label="提及量" width="100" sortable align="center" />
  </el-table>

...
export type EventDetailTabKey = 'event' | 'handle' | 'voice' | 'record' | 'related'
...
const activeKey = computed<EventDetailTabKey>(() => {
  ...
  return (matched?.key || props.tabs[0]?.key || 'event') as EventDetailTabKey
})
...
.event-detail-tabs {
  ...
  background: linear-gradient(180deg, #398BFF 0%, #398BFF 100%);
}

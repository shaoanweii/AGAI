...
const chartData = computed(() => {
  ...
  const positiveMentions = trendData.map(item => ({
    ...item,
    name: item.date,
    value: item.positiveMentions
  }))

  const neutralMentions = trendData.map(item => ({
    ...item,
    name: item.date,
    value: item.neutralMentions
  }))

  const negativeMentions = trendData.map(item => ({
    ...item,
    name: item.date,
    value: item.negativeMentions
  }))

  const totalMentions = trendData.map(item => ({
    ...item,
    name: item.date,
    value: item.totalMentions
  }))

  const negativeRates = trendData.map(item => ({
    ...item,
    name: item.date,
        ...
        const dataRows = [
          {
            ...
            mom: fmtFix(data.negativeRateMoM),
            yoy: fmtFix(data.negativeRateYoY)
          },
          {
            name: '正面提及量',
            value: fmtNum(data.positiveMentions),
            mom: fmtFix(data.positiveMentionsMoM),
            yoy: fmtFix(data.positiveMentionsYoY)
          },
          {
            name: '中性提及量',

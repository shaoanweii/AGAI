...
import type { CustomerTagExperienceVo } from '@/api/common/index.d'

echarts.use([BarChart, LineChart, TooltipComponent, GridComponent, LegendComponent, CanvasRenderer])

// 组件名称定义
defineOptions({
  name: 'BarAndPointChart'
})

// Props 接口定义
interface Props {
  data: CustomerTagExperienceVo[]
  width?: string | number
...
const renderChart = () => {
  ...
  if (!chartInstance) {
    ...
    chartInstance.on('click', (params: any) => {
      console.log('BarAndPointChart 点击事件:', params)
      if (params.componentType === 'series' && params.seriesType === 'bar') {
        console.log('发出barClick事件，参数名称:', params.name)
...
const getChartOption = (): EChartsCoreOption => {
  const xData = props.data.map(item => item.customerTagName)
  const positive = props.data.map(item => item.positiveMentions)
  const neutral = props.data.map(item => item.neutralMentions)
  const negative = props.data.map(item => -Math.abs(item.negativeMentions || 0))
  const experience = props.data.map(item => item.experienceValue)
  ...
  return {
    ...
    series: [
      ...
      },
      {
        ...
        symbol: 'circle',
        symbolSize: 9,
        lineStyle: { type: 'dashed', color: '#5D7092' },
        itemStyle: { borderWidth: 2, color: 'rgba(93, 112, 146, .3)', borderColor: '#5D7092' }
      }
    ]
  }
}

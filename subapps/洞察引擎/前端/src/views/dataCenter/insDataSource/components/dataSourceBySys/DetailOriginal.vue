...
import { exportSIRawData } from '@/api/dataCenter'
...
const {
  ...
  {
    method: 'POST',
    url: '/insights/insDataSource/getSIRawData'
  },
  res => {
    // 排除已知字段， 动态展示未知字段
    const whiteList = [
...
//   exportFile(exportSIRawData, table.filter)
// }, 300)
const handleExport = () => {
  showVisible()
}
    ...
    >
      ...
      <template v-if="dynamicColumn.length > 0">
        <template v-for="(key, index) in dynamicColumn" :key="index">
          <el-table-column :prop="`${key}`" :label="`${key}`" show-overflow-tooltip min-width="180">
      ...
      <el-table-column prop="dataStatus" label="数据状态" min-width="120">
        <template #default="{ row, $index }">

...
import { getSIDataSourceSearchCriteria } from '@/api/dataCenter'
...
const handleOpen = async () => {
  console.log('curDataSourceDetail.value', curDataSourceDetail.value)

  try {
    otherConditions.value = await getSIDataSourceSearchCriteria({
      clientId: userStore.clientId!,
      dataSourceId: curDataSource.value.id,
      batchId: curDataSourceDetail.value?.batchId,
      dataName: curDataSourceDetail.value?.dataName
    }).then(res => res.result)
  } catch (e) {}
  handleTypeChange(1)
}

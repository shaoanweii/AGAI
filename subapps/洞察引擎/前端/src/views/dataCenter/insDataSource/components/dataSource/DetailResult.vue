...
import { exportRawDataResult } from '@/api/dataCenter'
...
const {
  ...
  {
    method: 'POST',
    url: '/insights/insDataSource/getRawDataResult'
  },
  res => {
    // 排除已知字段， 动态展示未知字段
    const whiteList = [
...
//   if (!table.filter?.brandCode || !table.filter?.brandCode?.length) return []
//   return otherConditions.value?.brandCarModelList?.reduce((pre: any[], cur: any) => {
//     if (table.filter?.brandCode?.includes(cur.brandName)) {
//       pre = [...pre, ...cur.carName]
//     }
//     return pre
//   }, [])
...
//   exportFile(exportRawDataResult, table.filter)
// }, 300)
...
<template>
      ...
      <el-row class="w-full" :gutter="24">
        ...
        <el-col :span="8">
          <el-form-item label="标签分类">
            ...
            /> -->
            <!-- <el-cascader
              v-model="table.filter.qualityEndTag"
              :options="otherConditions?.qy"
              :props="{
                value: 'tagName',
                label: 'tagName',
                children: 'child'
              ...
              placeholder="质量标签"
              multiple
              :data-testid="`dataSource-result-10005`"
            /> -->
            <!--<el-select v-model="table.filter.formatFilters" placeholder="全部" clearable style="width: 104px">-->
  ...
  <div class="table" :style="computedCardHeight(330)">
    ...
    >
      <!-- <el-table-column prop="channelName" label="渠道" width="180">
        <template #default="{ row, $index }">
          <span :data-testid="`dataSource-result-t0-${$index}`">{{ row.channelName }}</span>
        </template>
      </el-table-column> -->
      <!--<el-table-column prop="content" label="声音ID"/>-->
      ...
      <el-table-column prop="labelTypeName" label="标签分类" width="180">
        <template #default="{ row, $index }">
      ...
      <!-- <el-table-column prop="labelTypeLevelFive" label=" 五级标签" width="180">
        <template #default="{ row, $index }">
          <span :data-testid="`dataSource-result-t73-${$index}`">{{
            row.labelTypeLevelFive
          }}</span>
        </template>
      </el-table-column> -->

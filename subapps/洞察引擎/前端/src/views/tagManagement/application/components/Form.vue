<template>
  ...
  >
    ...
    <el-form ref="formRef" :rules="rules" :model="formData" :style="{ width: '884px' }">
      <!--<el-form-item prop="appClient" label="应用客户">-->
      <!--  <el-select-->
      <!--    :data-testid="`application-form-10001`"-->
      <!--    v-model="formData.appClient"-->
      <!--    placeholder="请选择"-->
      <!--    :disabled="form.operation == 'edit'"-->
      <!--    clearable-->
      <!--    style="width: 100%"-->
      <!--    @change="appClientChange"-->
      <!--  >-->
      <!--    <el-option v-for="(item, index) of conditions.client" :data-testid="`application-form-10001-${index}`" :key="item.key" :label="item.value" :value="item.key"/>-->
      <!--  </el-select>-->
      <!--</el-form-item>-->
      ...
      <!--分类-->
      <template v-if="formData.tagAttribute === 'Category'">
        <el-form-item prop="tagName" label="分类名称">
          <el-input
            :data-testid="`application-form-10008`"
            v-model.trim="formData.tagName"
            :maxlength="50"
            clearable
            placeholder="请输入"
        ...
        <el-form-item prop="tagNameEn" label="英文名称">
          <el-input
            :data-testid="`application-form-10009`"
            v-model.trim="formData.tagNameEn"
            :maxlength="50"
            clearable
            placeholder="请输入"
        ...
        <el-form-item label="所属分类">
          <el-cascader
            :data-testid="`application-form-10010`"
            v-model="formData.tagParentId"
            :options="categoryTree"
        ...
        <el-form-item prop="tagStatus" label="是否启用">
          <el-radio-group :data-testid="`application-form-10011`" v-model="formData.tagStatus">
            <el-radio
              v-for="(item, index) of conditions.stopOrEnable"
              :key="index"
              :data-testid="`application-form-10011-${index}`"
              :value="item.key"
              >{{ item.value }}
            </el-radio>
          </el-radio-group>
...
// watch(() => userStore.clientId, (nval: string) => {
//   if (nval) {
//     appClientChange()
//   }
// })

// const appClientChange = () => {
//   formData.tagParentId = '';
//   getCategoryTreeByTagType();
// };

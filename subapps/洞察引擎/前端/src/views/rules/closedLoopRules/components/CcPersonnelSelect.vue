...
//   { id, deptNo, deptName, isAll, userId, employeeId, userName }
...
const props = withDefaults(
  defineProps<{
    ...
    placeholder?: string
    multiple?: boolean
    allowDeptSelect?: boolean
    autoCloseOnSelect?: boolean
    hideDeptInDisplay?: boolean
  }>(),
  {
    maxDisplay: 1,
    placeholder: '请选择抄送人员',
    multiple: true,
    allowDeptSelect: true,
    autoCloseOnSelect: true,
    hideDeptInDisplay: false
  }
)
...
const ccDisplayText = computed(() => {
  ...
  const parts = list.map(i => {
    if (i.isAll) return `${i.deptName || ''}/全部`
    // 单选人员场景只需要突出人员信息，隐藏部门可减少审核/责任人字段的回显噪音。
    if (!i.isAll && props.hideDeptInDisplay) {
      return i.userName ? `${i.userName}${i.employeeId ? '-' + i.employeeId : ''}` : ''
    }
    if (!i.isAll) return i.deptName + (i.userName ? '/' + `${i.userName}-${i.employeeId}` : '')
  })
  const max = props.maxDisplay
  if (parts.length > max) {
    const visible = parts.slice(0, max)
    const remaining = parts.length - max
...
const fetchDeptEmployeeTree = async () => {
  try {
    // 数据源统一从用户 store 的缓存获取（rulesActions 内部已做封装）
    const list = await rulesActions.updateDeptEmployeeTree()
    buildCcTree(list)
    await nextTick()
...
const keysFromModel = (list: any[]): string[] => {
  ...
  for (const item of list || []) {
    if (item?.isAll && item?.id) keys.push(`dept:${item.id}`)
    else if (!item?.isAll && item?.userId) keys.push(`user:${item.userId}`)
  }
  return keys
}

/**
 * 统一将树节点元数据转换为外层依赖的抄送结构，便于单选和多选共用同一套回填口径。
 * @param meta 树节点元数据
 * @returns 标准化的人员选择结果
 */
const buildSelectionByMeta = (meta: any) => {
  if (!meta) return []
  ...
  return [
    {
      id: meta?.parentDept?.id || '',
      deptNo: meta?.parentDept?.deptNo || '',
      deptName: meta?.parentDept?.deptName || '',
      isAll: false,
      userId: meta.userId,
...
const syncTreeCheckedByModel = async () => {
  ...
  ccCurrentKey.value = currentKey
  ccTreeRef.value?.setCurrentKey?.(currentKey || undefined)
}

/**
 * 单选模式下只允许保留一个对象；当业务要求必须选人时，可通过 allowDeptSelect 拦截部门节点。
 * @param data 当前点击的树节点
 */
const onCcCurrentChange = (data: any) => {
  if (props.multiple || !data) {
    return
  }

  const meta = ccNodeMap.value[data.key]
  if (!meta) {
    return
  }

  if (meta.type === 'dept' && !props.allowDeptSelect) {
    nextTick(() => {
      ccTreeRef.value?.setCurrentKey?.(ccCurrentKey.value || undefined)
    })
    return
  }

  const result = buildSelectionByMeta(meta)
  ccCurrentKey.value = data.key
  model.value = result
  emit('change', result)
  // 单选自动关闭改为显式配置，便于后续复用该组件时按页面交互策略灵活控制
  if (props.autoCloseOnSelect) {
    ccPanelVisible.value = false
  }
}
...
watch(
  ...
  (val, oldVal) => {
    ...
    if (list.length > MAX_CC_COUNT) {
      ElMessage.warning(`最多只能选择${MAX_CC_COUNT}个抄送对象`)
...
const clearSelection = () => {
  if (props.multiple) {
    ...
    ccCurrentKey.value = ''
    ccTreeRef.value?.setCurrentKey?.(undefined)
  }
  model.value = []
  emit('change', [])
}
  ...
  >
    ...
    <div class="cc-panel">
      ...
      <el-tree-v2
        ...
        :current-node-key="ccCurrentKey"
        :height="300"
        @check="onCcCheck"
        @current-change="onCcCurrentChange"
      >
      ...
      <div v-if="props.multiple" class="cc-panel__footer">

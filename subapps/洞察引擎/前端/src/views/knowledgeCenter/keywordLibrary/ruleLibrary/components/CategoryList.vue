...
const deleteItem = async (item: any) => {
  try {
    await ElMessageBox.confirm(`确认要删除分组「${item.name}」吗？`, '删除确认', {
    ...
    const response = await insDataResourceDelete({ id: item.id, type: 'rule' })
...
<template>
  <div class="flex-col h-full">
    <div class="flex-between items-center mb-24">
      <div class="header-title-class">规则分组</div>
    ...
    <div class="flex-1 flex-col">
      <div class="h-55 flex-y-center title-class pl-16 pr-16">分组列表</div>
            ...
            >
              <div class="flex-y-center">
                <SvgIcon
                  ...
                  "
                />
                <div class="category-name ml-8">{{ item.name }}</div>
              </div>

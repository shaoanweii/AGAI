...
import { batchUpdateBrandStatus, brandSeriesEnableStatus } from '@/api/brandSeries'
...
const {
  ...
  res => {
    ...
    // 兼容后端返回字段差异，避免列表因字段命名不一致出现空值
    const list = (pageResult.list || pageResult.records || []).map((item: any) => ({
      ...item,
      automark: item.automark || item.automarkName || ''
    }))
    return {
      list,
        ...
        <el-row class="w-full" :gutter="16">
          <el-col :span="6">
            <el-form-item label="车企">
              <el-select-v2
                v-model="table.filter.automarkList"
                placeholder="请选择"
                clearable
                filterable
                multiple
                collapse-tags
                :options="getConditionOptions('automark')"
  ...
  >
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="name" label="品牌" min-width="160" show-overflow-tooltip />
        <el-table-column
          prop="automark"
          label="车企"
          min-width="160"
    ...
    >
      <template v-if="batchDialogAction === 'enable'">是否确认批量启用选中品牌？</template>
      <template v-else>是否确认批量禁用选中品牌？</template>
    </AppDialog>

...
import { batchUpdateSeriesStatus, brandSeriesEnableStatus } from '@/api/brandSeries'
...
const { conditions } = useConditions({ url: '/insights/carSeriesInfo/conditions' })
...
const {
  ...
  res => {
    ...
    return {
      list: pageResult.list || pageResult.records || [],
...
const isNewCarOptions = computed(() => getConditionOptions('isNewCar'))
        ...
        <el-row class="w-full" :gutter="16">
          <el-col :span="6">
            <el-form-item label="品牌">
              <el-select-v2
                v-model="table.filter.brandCodes"
                placeholder="请选择"
                clearable
                filterable
                multiple
                collapse-tags
                :options="getConditionOptions('brand')"
          ...
          <el-col :span="6">
            <el-form-item label="是否新车">
              <el-select v-model="table.filter.isNewCar" placeholder="不限" clearable filterable>
                <el-option
                  v-for="(item, index) in isNewCarOptions"
                  :key="`${item.key}-${index}`"
                  :label="item.value"
                  :value="item.key"
                />
  ...
  >
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="name" label="车系" min-width="160" show-overflow-tooltip />
        <el-table-column
          prop="brandName"
          label="品牌"
          min-width="160"
    ...
    >
      <template v-if="batchDialogAction === 'enable'">是否确认批量启用选中车系？</template>
      <template v-else>是否确认批量禁用选中车系？</template>
    </AppDialog>
    ...
    <SeriesFormDialog
      ...
      :is-new-car-options="isNewCarOptions"

    ...
    <template #default>
      <div class="body-wrapper">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" class="form">
          ...
          <el-form-item v-if="viewStatus === 'edit'" label="渠道编码">
            <span>{{ editRecord?.code || '' }}</span>
          </el-form-item>
          <el-form-item prop="parentId" label="所属分类">
            <el-cascader
              ...
              :format-label="(options: any) => options.map((option: any) => option.name)?.join('#')"
              check-strictly
              style="width: 100%"
              placeholder="请选择"
              clearable
...
import type { CategorizeItem, ChannelParams } from '@/types/baseSeting.types'
import { saveChannel, updateChannel } from '@/api/baseSettings'
...
const { type, viewStatus, categorizeTree, editRecord } = toRefs(props)
...
const editInit = () => {
  ...
  if (type.value === 1 && editRecord.value.id !== '-1') {
    form.parentId = editRecord.value.parentId === '0' ? undefined : editRecord.value.parentId
  }
}
const handleOpen = () => {
  if (type.value === 1) {
  ...
  try {
    ...
    if (valid) {
      if (
        ...
        return ElMessage.error('当前渠道分类仅支持五级分类，请修改后重新提交。')
      }
      const api = viewStatus.value === 'add' ? saveChannel : updateChannel

      const params = cloneDeep(form)
      params.parentId = form.parentId || '0'
      api(params)
        .then(res => {
          if (res.code === '200') {
            emit('refreshList')

    ...
    >
      <div class="list-item flex item-center">
        <div :style="{ width: '16px', marginLeft: (item?.level || 0) * 16 + 'px' }">
          <template v-if="item.child">
            <i
              v-if="!item.showChild"
              :data-testid="`baseSetting-region-left-icon1-${index}`"
            ...
            <i
              ...
              :data-testid="`baseSetting-region-left-icon2-${index}`"
              @click.stop="showChild(item, index)"
            ></i>
          </template>
        </div>
        <span :data-testid="`baseSetting-region-left-10002-${index}`" style="margin-left: 8px">{{
          item?.name
        }}</span>
      </div>
      <div class="list-item-actions">
        <!-- <icon-edit
          ...
          v-if="item.id !== '-1'"
          :data-testid="`baseSetting-region-left-10003-${index}`"
          @click.stop="handleEdit(item)"
        />
        <icon-delete
          v-auth="`settings-channelConfig-delete`"
          :data-testid="`baseSetting-region-left-10004-${index}`"
          :class="{ 'ft-disabled': false }"
          v-if="item.id !== '-1'"

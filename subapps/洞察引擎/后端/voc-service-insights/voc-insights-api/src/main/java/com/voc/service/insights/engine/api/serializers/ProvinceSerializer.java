...
public class ProvinceSerializer extends JsonSerializer<Object> implements ContextualSerializer {


    private static final Logger log = LoggerFactory.getLogger(ProvinceSerializer.class);
    String code;
    String defaultValue;
    ...
    public void serialize(Object value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        try {
            ...
            Optional<ProvinceAreaVo> vo = list.stream().filter(item -> item.getProvinceCode().equalsIgnoreCase(String.valueOf(value))).findFirst();
            if (vo.isPresent()) {
                gen.writeString(String.valueOf(value));
                gen.writeFieldName(gen.getOutputContext().getCurrentName().concat("Text"));
                gen.writeString(vo.get().getProvinceName());
...
//            log.error(e.getMessage(), e);
    ...
    public JsonSerializer<?> createContextual(SerializerProvider prov, BeanProperty property) throws JsonMappingException {
        Province annotation = property.getAnnotation(Province.class);
        // 只针对String类型属性进行脱敏
        if (Objects.nonNull(annotation) && Objects.equals(String.class, property.getType().getRawClass())) {
            ...
            log.error("{}属性类型不支持，当前只支持字符串类型，请注意！", Province.class.getSimpleName());
        }

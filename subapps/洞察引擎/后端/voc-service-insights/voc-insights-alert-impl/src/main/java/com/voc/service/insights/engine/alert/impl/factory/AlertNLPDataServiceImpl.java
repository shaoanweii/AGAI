...
import com.voc.service.insights.engine.alert.mapper.AltNLPDataMapper;
...
/**
 * @Title: InsAlertNLPDataServiceImpl
...
@DS("starrocks1")
@AllArgsConstructor
public class AlertNLPDataServiceImpl extends ServiceImpl<AltNLPDataMapper, AltMonitoringDataEntity>
        implements IInsAlertNLPDataService {
    private static final Logger log = LoggerFactory.getLogger(AlertNLPDataServiceImpl.class);
    @Getter
    final String dataType = AlertTaskEnum.NLP_DATA.getCode();
...
//            final List<AltMonitoringDataModel> list = pullData.stream().map(convertService::toModel).collect(Collectors.toList());
    ...
    // nlp数据 =（今日原始数据数量/今日NLP返回声音数-历史比值均值）/历史比值均值 * 100%
    // 历史比值均值 = 原始数据的历史比值的平均值
    @Override
    public List<InsAltDataModel> alertBarChart(AltAlarmDataModel altAlarmDataModel) {
        return monitoringDataService.nlpDataAlertBarChart(altAlarmDataModel);
    }

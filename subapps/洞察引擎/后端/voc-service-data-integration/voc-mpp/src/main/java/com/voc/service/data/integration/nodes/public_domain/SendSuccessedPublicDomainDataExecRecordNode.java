 ...
 * @Version:1.0
 */
@LiteflowComponent(id = "sendSuccessedPublicDomainDataExecRecordNode", name = "推送数据到公域执行记录表消息队列")
public class SendSuccessedPublicDomainDataExecRecordNode extends NodeComponent {
    public static final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss", Locale.CHINA);
    private static final Logger log = LoggerFactory.getLogger(SendSuccessedPublicDomainDataExecRecordNode.class);
    ...
    public void process() throws Exception {
        ...
        try {
            if (CollUtil.isEmpty(context.getSuccessfulDataset())) {
                ...
                final List<DataIntegrationRecordModel> list1 = list.stream().map(model -> {
                ...
                log.info("【{}】保存记录成功数据: {}", context.getWorkId(), list1.size());
                publicDomainService.saveRecordList(context.getClientId(), list1);
            }
        } catch (Exception e) {

 ...
 * @Version:1.0
 */
@LiteflowComponent(id = "recordfailurePublicDomainDatasetNode", name = "记录失败数据节点-公域")
public class RecordfailurePublicDomainDatasetNode extends NodeComponent {
    private static final Logger log = LoggerFactory.getLogger(RecordfailurePublicDomainDatasetNode.class);
    @Autowired
    IPublicDomainService errorDataMsgService;
    ...
    public void process() throws Exception {
        ...
        try {
            if (CollUtil.isNotEmpty(context.getFailedDataset())) {
                log.info("【{}】数据验证失败，失败数据：{}", context.getWorkId(), context.getFailedDataset().size());
                //保存记录异常数据集
                this.saveFailedData(context.getClientId(), context.getWorkId(), context.getChannelType(), context.getFailedDataset());
            } else {
                log.info("【{}】数据验证完成，本次无异常数据 ", context.getWorkId());
            }
        } catch (Exception e) {
...
//            model.setRetryCount(retryCountMap.containsKey(item.getId()) ? retryCountMap.get(item.getId()) +1 : 1);
        ...
        errorDataMsgService.saveRecordList(clientId, list);
    }

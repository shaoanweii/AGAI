...
public class YouBianCodeUtil {
    ...
    private static final String codePrefix = "A";
    ...
    public static synchronized String getSubYouBianCode(String parentCode, String localCode) {
        return getSubYouBianCode(parentCode, localCode, null);
    }

    /**
     * 根据父code和同级最新code生成下一个code，若生成后已存在则继续递增，直到唯一
     *
     * @param parentCode         上级code
     * @param localCode          同级最新code
     * @param codeExistsPredicate code存在校验
     * @return
     */
    public static synchronized String getSubYouBianCode(String parentCode, String localCode, Predicate<String> codeExistsPredicate) {
        String nextCode;
        if (StrUtil.isNotBlank(localCode)) {
//			return parentCode + getNextYouBianCode(localCode);
            nextCode = getNextYouBianCode(localCode);
        } else {
            nextCode = parentCode + codePrefix + getNextStrNum(0);
        }
        while (codeExistsPredicate != null && codeExistsPredicate.test(nextCode)) {
            nextCode = getNextYouBianCode(nextCode);
        }
        return nextCode;
    }
    ...
    public static String[] cutYouBianCode(String code) {
        if (code == null || StrUtil.isNotBlank(code)) {
            return null;
        } else {

 ...
 * @Version:1.0
 */
public class HutoolStopWatch {
    private static final Logger logger = LoggerFactory.getLogger(HutoolStopWatch.class);
    ...
    public static HutoolStopWatch create(String id) {
        return new HutoolStopWatch(id);
    }

    public HutoolStopWatch() {
        this("");
    }

    public HutoolStopWatch(String id) {
        this(id, true);
    }

    public HutoolStopWatch(String id, boolean keepTaskList) {
        this.id = id;
        if (keepTaskList) {
    ...
    public HutoolStopWatch.TaskInfo getLastTaskInfo() throws IllegalStateException {
        if (this.lastTaskInfo == null) {
    ...
    public HutoolStopWatch.TaskInfo[] getTaskInfo() {
        if (null == this.taskList) {
            throw new UnsupportedOperationException("Task info is not being kept!");
        } else {
            return (HutoolStopWatch.TaskInfo[]) this.taskList.toArray(new HutoolStopWatch.TaskInfo[0]);
        }
    }
...
//        sb.append(FileUtil.getLineSeparator());
        if (null == this.taskList) {
            ...
            logger.debug("---------------------------------------------");
            logger.debug("{}{}", DateUtil.getShotName(unit), "         %     Task name");
            logger.debug("---------------------------------------------");
            ...
            HutoolStopWatch.TaskInfo[] var5 = this.getTaskInfo();
            int var6 = var5.length;

            for (int var7 = 0; var7 < var6; ++var7) {
                StringBuilder sb = new StringBuilder();
                HutoolStopWatch.TaskInfo task = var5[var7];
...
//        logger.info("{}",this.shortSummary(TimeUnit.SECONDS));
        logger.info("{}",this.shortSummary(TimeUnit.MILLISECONDS));
    ...
    public String toString() {
        ...
        if (null != this.taskList) {
            ...
            while (var2.hasNext()) {
                HutoolStopWatch.TaskInfo task = (HutoolStopWatch.TaskInfo) var2.next();

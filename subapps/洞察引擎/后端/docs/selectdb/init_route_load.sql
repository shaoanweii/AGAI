...
(
    ...
    "property.group.id" = "voc-analysis-sdb-test",
...
PROPERTIES (
    "desired_concurrent_number" = "2",           -- 并发数别太高
    "max_batch_interval" = "5",                 -- 秒
    "max_batch_rows" = "200000",                 -- 行数
    "max_batch_size" = "1073741824",             -- 1GB/批
    "exec_mem_limit" = "2147483648" ,             -- 2GB/任务
...
-- show ROUTINE LOAD
-- stop ROUTINE LOAD for voc_imp_hudi_dm_voc_cust_vehicle_rel_kafka
CREATE ROUTINE LOAD voc_imp_hudi_dm_voc_cust_vehicle_rel_kafka ON voc_imp_hudi_dm_voc_cust_vehicle_rel
COLUMNS(
    vin, idcard, w_insert_dt, w_update_dt, job_name, ds
)
PROPERTIES (
    "desired_concurrent_number" = "3",           -- 并发数别太高
    "max_batch_interval" = "5",                 -- 秒
...
FROM KAFKA (
    "kafka_broker_list" = "10.62.133.17:29095",
    "kafka_topic" = "voc_imp_hudi_dm_voc_cust_vehicle_rel",
    "kafka_partitions" = "0,1,2",
    "property.group.id" = "voc-analysis-sdb-test",
    "property.kafka_default_offsets" = "OFFSET_BEGINNING"
);
...
(
 ...
 CREATE ROUTINE LOAD voc_anal_flow_mate_data_full_ext_kafka ON voc_anal_flow_mate_data_full_ext
 COLUMNS(id, create_time, data_create_time, content_type, data_update_time, data_id,client_id,
 channel_code, brand, series, model, is_outer, one_id, id_car_no, mobile, email, global_id,
 user_id, user_name, vhl_id, vhl_vin, dlr_id, dlr_code, dlr_type, market_id, title, content,
 is_wsater_army, weight, attrs, attrs2, attrs3, work_id, done, model_type,
 cust_ext_attrs,dealer_ext_attrs,prd_ext_attrs,vhl_ext_attrs,data_status,ds,abandon,insert_dt)
 PROPERTIES
 (
     "enclose"="\"",
     "escape"="\\",
    ...
    "exec_mem_limit" = "2147483648",              -- 2GB/任务
     "format" = "json",
     "jsonpaths" = "[\"$.id\",\"$.createTime\", \"$.dataCreateTime\",\"$.contentType\",\"$.dataUpdateTime\",\"$.dataId\",\"$.clientId\",
                    \"$.channelCode\",\"$.brand\", \"$.series\",\"$.model\",\"$.isOuter\",\"$.oneId\",\"$.idCarNo\", \"$.mobile\",\"$.email\",\"$.globalId\",
                    \"$.userId\",\"$.userName\",\"$.vhlId\", \"$.vhlVin\",\"$.dlrId\",\"$.dlrCode\",\"$.dlrType\",\"$.marketId\", \"$.title\",\"$.content\",
                    \"$.isWsaterArmy\",\"$.weight\",\"$.attrs\",\"$.attrs2\", \"$.attrs3\",\"$.workId\",\"$.done\",\"$.modelType\",
					\"$.custExtAttrs\",\"$.dealerExtAttrs\",\"$.prdExtAttrs\",\"$.vhlExtAttrs\", \"$.dataStatus\",\"$.ds\",\"$.abandon\",\"$.insertDt\"
					]"
  )
  FROM KAFKA
  (
       "kafka_broker_list" = "172.16.80.16:30092,172.16.80.16:31673,172.16.80.16:32705",
        "kafka_partitions" = "0,1,2",
      "kafka_topic" = "voc_anal_flow_mate_data_ext",
      "property.group.id" = "voc-analysis-sdb-test",
      "property.kafka_default_offsets" = "OFFSET_END"
  );

 ...
 * @Version:1.0
 */
@LiteflowComponent(id = "saveModelLabelDataNode", name = "保存模型已打标数据节点")
public class SaveModelLabelDataNode extends AbstractNode {

    private static final Logger log = LoggerFactory.getLogger(SaveModelLabelDataNode.class);
    @Autowired
    IAysModelResltAnalysisService iAysModelResltAnalysisService;
    @Value("${analysis.flow.nodes.saveModelLabelDataNode.enable:false}")
    boolean enable;
    ...
    public void process() throws Exception {
        try {
            ...
            iAysModelResltAnalysisService.saveBatchAnalysis(clientId, modelLabelDataList);
        } catch (Exception e) {
